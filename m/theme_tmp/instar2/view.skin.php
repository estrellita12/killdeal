...
<div class="sp_wrap">
	...
	<?php if(!$is_only) { ?>
		...
		<div class="sp_tbox">
			<ul>
				<li class='tlst'>할인코드</li>
				<li class='trst'>
					<div class='trst-code'><?php echo $pt_id; ?></div>
				</li>
			</ul>			
			<ul>
...
echo 
	'<script>
            ...
            $(".mpr").append(`<span> -> </span>`);
			$(".mpr").css("font-weight","normal");		
			$("#d2").css("display","none");					
			$("#d3").css("display","none");	
            $(".wish_btn").css("display","none");				
			$(".sp_btn").children().css("width","50%");				
            $(".mpr").text(`${number_format(Math.ceil(Number(over_estimated_pr.replaceAll(",","").replaceAll("원","")) / 0.9 / 100) * 100)}원`);
            $(".mpr").append(`<span> --> </span>`);
            $(".mpr").append(`<span style="color:red">${over_estimated_pr}</span>`);
            $(".mpr").append(`&nbsp;&nbsp;&nbsp;<span style="color:white; background: black">10% 추가할인★</span>`);
	</script>';
?>