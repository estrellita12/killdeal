...
<div id="sod_fin">
	...
	<section id="sod_fin_list">
        ...
        <ul id="sod_list_inq" class="sod_list">
			...
			for($k=0; $ct=sql_fetch_array($res); $k++) {
				...
				if($rw['dan'] == 5) {
					if(is_null_time($rw['user_date'])) {
						...
						$li_btn .= '<a href="'.TB_MSHOP_URL.'/orderchange.php?gs_id='.$rw['gs_id'].'&od_id='.$rw['od_id'].'&q=반품" target="_self" class="btn_ssmall bx-white">반품신청</a>'.PHP_EOL;
						$li_btn .= '<a href="'.TB_MSHOP_URL.'/orderchange.php?gs_id='.$rw['gs_id'].'&od_id='.$rw['od_id'].'&q=교환" target="_self" class="btn_ssmall bx-white">교환신청</a>'.PHP_EOL;
					}
					//$li_btn .= '<a href="'.TB_MSHOP_URL.'/view_user_form.php?gs_id='.$rw['gs_id'].'" onclick="win_open(this, \'winorderreview\');return false;" class="btn_ssmall bx-white">리뷰작성</a>'.PHP_EOL;
					$li_btn .= '<a href="'.TB_MSHOP_URL.'/view_user_form.php?gs_id='.$rw['gs_id'].'" target="_self" class="btn_ssmall bx-white">리뷰작성</a>'.PHP_EOL;
				}
			...
			<li class="sod_li">
                ...
                <div class="li_prqty">
                    ...
                    <span class="prqty_stat li_prqty_sp"><span>상태 </span><p style="color:red"><?php echo $gw_status[$rw['dan']]; ?></p></span>
                </div>
				...
				<?php if($dlcomp[1] && $rw['delivery_no']) {
					echo get_delivery_inquiry($rw['delivery'], $rw['delivery_no'], 'btn_ssmall bx-white');
				} ?>
				</div>
				<?php } ?>
            </li>
		...
		<dl id="sod_bsk_tot">
            ...
            <dt class="sod_bsk_dvr"><span>기본금결제</span></dt>
            <dd class="sod_bsk_dvr"><strong><?php echo display_price($stotal['usepoint2']); ?></strong></dd>
            <?php } ?>
		...
		<div class="odf_tbl">
			...
			<tr>
				...
				<td>
					...
					if($od['paymethod'] == '신용카드' || $od['paymethod'] == '복지카드' || $od['paymethod'] == '삼성페이')
					{
						if($od['od_pg'] == 'lg') {
							require_once TB_SHOP_PATH.'/settle_lg.inc.php';
							$LGD_TID      = $od['od_tno'];
			...
			<tr>
				...
				?>
					...
					<a href="<?php echo TB_MSHOP_URL; ?>/taxsave.php?od_id=<?php echo $od_id; ?>" target="_self" class="btn_small">현금영수증 발급하기</a>
				<?php } ?>
				</td>
			</tr>
			<?php
	...
	<section id="sod_fin_cancel">
		...
		<div id="sod_fin_cancelfrm">
			...
			<!-- 20200221 기존 text 취소사유 주석 select로 변경
