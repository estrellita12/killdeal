		...
		<ul class="sod_list">
			...
			for($i=0; $row=sql_fetch_array($result); $i++) {
				...
				if($pt_id == 'dodamgolf')
				{
			        ...
			        //$point = floor($sum['price'] * 0.03);//포인트적립출력_20200108
					$point = $sum['point'];
		...
		<div class="odf_tbl">
			...
			<tr>
			    ...
			    <?php }else if($pt_id == 'golfu'){ ?>
                     <input type="text" name="cellphone" value="<?php echo $_COOKIE['UserMobile']; ?>" onKeyup="PhoneNumberSplit(this);" required class="frm_input required" maxlength="20"> 
				<?php }else{ ?> 
...
<!-- 			<tr> -->
<!-- 				<td><p class="required">전화번호</p><input type="text" name="telephone" value="<?php echo $member['telephone']; ?>" class="frm_input" maxlength="20"></td> -->
<!-- 			</tr> -->
			<tr>
				<td>
			...
			<tr>
				...
				<?php }else{ ?> 
<!-- 20200811이메일 셀렉추가로 폼변경 	   <input type="text" name="email" value="<?php echo $member['email']; ?>" required class="frm_input required wfull" >  -->
...
<!-- 			</tr> -->
	...
	if($default['de_bank_use'])
		...
		$multi_settle .= "<input type='radio' name='paymethod' value='무통장'   onchange='calculate_paymethod(this.value);'><label for='sel_pay2' class='css-label '>무통장입금</label>";
	if($default['de_card_use'])
		//$multi_settle .= "<option value='신용카드'>신용카드</option>\n";
		$multi_settle .= "<input type='radio' name='paymethod' value='신용카드'  onchange='calculate_paymethod(this.value);' checked>신용카드 &nbsp;";
	if($default['de_hp_use'])
		//$multi_settle .= "<option value='휴대폰'>휴대폰</option>\n";
		$multi_settle .= "<input type='radio' name='paymethod' value='휴대폰' id='sel_pay4' class='css-checkbox lrg' onchange='calculate_paymethod(this.value);'><label for='sel_pay4' class='css-label '>휴대폰</label>";
	if($default['de_iche_use'])
		//$multi_settle .= "<option value='계좌이체'>".$escrow_title."계좌이체</option>\n";	
		$multi_settle .= "<input type='radio' name='paymethod' value='계좌이체'  onchange='calculate_paymethod(this.value);'>계좌이체 &nbsp;";
	...
	if($default['de_vbank_use'])
		...
		$multi_settle .= "<input type='radio' name='paymethod' value='가상계좌'  onchange='calculate_paymethod(this.value);'>가상계좌 &nbsp;";
    /*
	골프유닷넷 포인트 전액결제 X_20200121
	...
	<section id="sod_frm_pay">
		...
		<div class="odf_tbl">
			...
			<tr>
				<td>
					<p>포인트결제</p>
					<input type="text" name="use_point" value="0" onkeyup="calculate_temp_point(this.value); this.value=number_format(this.value);" class="frm_input w100"> P
					<div>보유포인트 : <b><?php echo display_point($member['point']); ?></b>(1만포인트 이상 보유시 구매가의 10% 이내에서 <br>최대 1만포인트까지 100P단위로 사용가능)</div> <!-- 20200624 1만포인트 고정 -->
<!-- 				<div>잔액 : <b><?php echo display_point($member['point']); ?></b> (<?php echo display_point($config['usepoint']); ?> 이상 보유시 구매가의 10% 이내에서 <br>최대 1만포인트까지 100P단위로 사용가능)</div> -->
				</td>
			</tr>
			<?php } ?>
...
<div class="buy-btn_cont">
    <div id="sod_bsk_act" class="btn_confirm">
		...
		<input type="submit" value="결제하기" class="btn_medium wset">
    </div>
</div>
...
function calculate_order_price() {
	...
	*/
	...
	//$("input[name=tot_price2]").val(number_format(String(tot_price)));
	//20200109_tot_price2적용필요
}
