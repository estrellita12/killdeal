...
function reg_its_member() {
    ...
    webkit.messageHandlers.cordova_iab.postMessage(JSON.stringify({message: "reg"}));
}
function login_its_member(gs_id) {
	if(gs_id != null)
	{
		window.parent.postMessage({'message':'login;'+gs_id}, '*' );
        webkit.messageHandlers.cordova_iab.postMessage(JSON.stringify({message: "login;"+gs_id}));
	}
	else
	{
		window.parent.postMessage({'message':'login'}, '*' );
        webkit.messageHandlers.cordova_iab.postMessage(JSON.stringify({message: "login"}));
	}
}
</script>
...
<div id="slideMenu">
	...
	<dl class="top_btn gradient">
		...
		<dd>
		    ...
		    <?php } else {   //비로그인  ?>
			                <a href="" class="btn_medium bx-white" onClick="reg_its_member();">회원가입</a>
			<?php      } ?>
		</dd>
	</dl>
