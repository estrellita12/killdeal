...
<?php if($default['de_sns_login_use']) { ?>
	<div class="sns_box" style="margin : 0 15px; border:none; border-top:1px solid black;">
...
<div class="tbl_frm01 tbl_wrap">
	...
	<tr>
		...
		<td>
            <input type="password" name="mb_password" id="reg_mb_password"<?php echo $required; ?> class="frm_input<?php echo $required; ?>" size="20" maxlength="20">
            <span class="frm_info">영문, 숫자 ,특수문자 중 2종류 이상을 조합하여 8자리 이상.</span> <!--2022-10-28 비밀번호 유효성 검사 추가-->
        </td>
	</tr>
	<tr>
...
function fregisterform_submit(f)
{
	...
	if(f.w.value == "") {
		if(f.mb_password.value.length < 8) { //2022-10-28 비밀번호 유효성 검사 추가
			alert("비밀번호를 8자리 이상 입력하여 주십시오."); 
			f.mb_password.focus();
			return false;
		}
	}
	...
	if(f.mb_password.value.length > 0) {
        ...
        */
        ...
        //2022-10-28 비밀번호 유효성 검사 추가
        let num = f.mb_password_re.value.search(/[0-9]/g);
        let eng = f.mb_password_re.value.search(/[a-z]/ig);
        let spe = f.mb_password_re.value.search(/[`~!@@#$%^&*|₩₩₩'₩";:₩/?]/gi);
        if(f.mb_password_re.value.length < 8){
            alert("비밀번호를 8자리 이상 입력하여 주십시오.");
			f.mb_password_re.focus();
			return false;
        }else if(f.mb_password_re.value.search(/\s/) != -1){
            alert("비밀번호는 공백 없이 입력하여 주십시오.");
			f.mb_password_re.focus();
			return false;
        }else if( (num < 0 && eng < 0) || (eng < 0 && spe < 0) || (spe < 0 && num < 0) ){
            alert("영문, 숫자, 특수문자 중 2가지 이상을 혼합하여 입력하여 주십시오.");
			f.mb_password_re.focus();
			return false;
        }
