...
?>
	...
	<?php if($pt_id == "golf") { 
		 ...
		 $chk_data = "D8C1D04BF3A5F912";
 
         $hdata = array(
             'mem_id' => $mem_id,
	    	 'shopevent_no' => $shopevent_no,
			 'proc_code' => $proc_code,
			 'chk_data' => $chk_data,
			 'media_cd' => 'MW'
        );

		$url = "https://mgiftdev.e-hyundai.com/hb2efront_new/pointOpenAPI.do?".http_build_query($hdata);
        ...
        curl_close($ch);
		//var_dump($load_string);//xml문서 그대로 출력
		echo $load_string->return_code,"<br>";//curl_magam.php에 적용할것(리턴코드 : 000)
		//$load_string->return_point :회원의 기본금
        /*
           if(기본금 >=결제금액)
		   {   
                기본금사용금액 = 결제금액
				사용후 기본금 잔여금액 =  기본금 - 결제금액
           }
		   else
		   {    기본금사용금액 = 보유기본금
		        사용후 기본금 잔여금액 = 0
...
<!--
		...
		$(document).ready(function(){
			...
			$.ajax({
					  ...
					  type : 'GET', 
		              url : 'https://mgiftdev.e-hyundai.com/hb2efront_new/pointOpenAPI.do',
					  data : {
			                     mem_id : 'B856E909E78FDFA2690CD9705BFD4D92',
                                 shopevent_no : '73FB843F801B403D809F23C00A436192',
								 proc_code : 'FA156C018605778E',
...
<div id="sod_frm">
	...
	<section id="sod_frm_pay">
		...
		<div class="odf_tbl">
			...
			<?php } ?>
			<tr>
				<th scope="row">기본금 사용</th>
				<td>
					<input type="text" id="use_point2" name="use_point2" value="0" onkeyup="calculate_temp_point(this.value); this.value=number_format(this.value);" class="frm_input w100"> 원
					 보유기본금<span id="hwelpoint"></span> 
				</td>
			</tr>
			<tr>
