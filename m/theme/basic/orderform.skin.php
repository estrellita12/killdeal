...
if($default['de_easy_pay_use']){ 
    ...
    if( in_array('naverpay', $de_easy_pay_service_array) ){
        $multi_settle .= "<div>";
        $multi_settle .= "<input type='radio' name='paymethod' value='간편결제' id='easy_naverpay' class='css-checkbox lrg' onchange='calculate_paymethod(\"easy_naverpay\");'>";
        $multi_settle .= "<label for='easy_naverpay' class='css-label '>네이버페이</label>";
        $multi_settle .= "</div>\n";
    }
    if( in_array('kakaopay', $de_easy_pay_service_array) ){
        $multi_settle .= "<div>";
        $multi_settle .= "<input type='radio' name='paymethod' value='간편결제' id='easy_kakaopay' class='css-checkbox lrg' onchange='calculate_paymethod(\"easy_kakaopay\");'>";
        $multi_settle .= "<label for='easy_kakaopay' class='css-label '>카카오페이</label>";
        $multi_settle .= "</div>\n";
    }
    if( in_array('tosspay', $de_easy_pay_service_array) ){
        $multi_settle .= "<div>";
        $multi_settle .= "<input type='radio' name='paymethod' value='간편결제' id='easy_tosspay' class='css-checkbox lrg' onchange='calculate_paymethod(\"easy_tosspay\");'>";
        $multi_settle .= "<label for='easy_tosspay' class='css-label '>토스페이</label>";
        $multi_settle .= "</div>\n";
    }
    $multi_settle .= "<input type='hidden' name='easy_pay_service' value=''>";
}
...
// 2024-01-29
if(getRadioVal(f.paymethod) == "간편결제"){
    var obj = f.paymethod;
    if (obj) {
        if (typeof(obj.length) == "undefined") {
            ...
            for (i=0; i<obj.length; i++) {
                if (obj[i].checked) {
                    f.easy_pay_service.value = obj[i].id;
                    break;
                }
            }
        }
...
function calculate_paymethod(type) {
    ...
    switch(type){
        case '카카오페이':
            $(".paymethod > div").css("background-color", "#FFFFFF");
        ...
        case 'easy_naverpay':
            $(".paymethod > div").css("background-color", "#FFFFFF");
            $("#easy_naverpay").parent().css("background-color", "#FFFFF0");
            break;
        case 'easy_kakaopay':
            $(".paymethod > div").css("background-color", "#FFFFFF");
            $("#easy_kakaopay").parent().css("background-color", "#FFFFF0");
            break;
        case 'easy_tosspay':
            $(".paymethod > div").css("background-color", "#FFFFFF");
            $("#easy_tosspay").parent().css("background-color", "#FFFFF0");
            break;
