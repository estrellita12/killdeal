...
<div class="timesale2">
    <div class="main_banner gradient"><img src="/img/timesale_header3_m.png"></div>
    <?php if(!$total_count || $total_count==0 ) { ?>
    <!-- (2020-12-24) 타임 세일 이미지 추가 -->
    <div class="empty_list bb">
        <img src="/img/timesale_soldout.png" style="width:100%">
    </div>
    <?php } ?>
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if($row['normal_price'] > $it_amount && !$is_uncase) {
			// $sett = (($row['normal_price'] - $it_amount) / $row['normal_price'] * 100)-10;
			// $sale = '<p class="sale">'.number_format($sett,0).'<span>%</span><span style="font-size:15px"> + 10%</span></p>';
                ...
                <div class="price">
                    <table>
                        <tr>
                            ...
                            <td class="tar tl"><?php echo $it_sprice; ?></td>
                        </tr>
                        <tr>
                            <td class="tar lh1"><?php echo $it_price; ?></td>
                        </tr>
                    </table>
                </div>
