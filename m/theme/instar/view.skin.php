...
<div class="sp_wrap">
	...
	<?php if(!$is_only) { ?>
        ...
        if(!$is_pr_msg && !$is_buy_only && !$is_soldout && $gs['normal_price']) { 
            ...
            $sale = '<span class="sale">'.number_format($sett,0).'<span>%</span></span>';            
            ?>
		<div class="sp_tbox">
			<ul>
				<li class='tlst'>시중가격</li>
				<li class='trst fc_137 tl' style="width:18% !important; "><?php echo display_price2($gs['normal_price']); ?></li>
                <li class='view_percent' style=" color:#333; font-weight:bold"><?php echo $sale;?></li>
			</ul>
		</div>
		<?php } ?>
		<div class="sp_tbox">
...
echo 
	'<script>
            $(".trst-amt .mpr").append("&nbsp;&nbsp;&nbsp;<span style=\'font-weight: lighter; font-size : 11px; background:black; color:white;\'> 추천인할인가★ &nbsp; </span>");                
			$(".mpr").css("font-weight","normal");		
			$("#d2").css("display","none");					
			$("#d3").css("display","none");					
            $(".wish_btn").css("display","none");
			$(".sp_btn").children().css("width","50%");				
	</script>';
?>
<script>
$(function(){
