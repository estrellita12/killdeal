...
$sql = " select count(*) as cnt from shop_member where email = '$email' and id = '$mb_id' ";
$row = sql_fetch($sql);
if($row['cnt'] > 1)
    alert('동일한 메일주소가 2개 이상 존재합니다.\\n\\n관리자에게 문의하여 주십시오.');

$sql = " select index_no, id, name, email from shop_member where email = '$email' and id = '$mb_id' ";
$mb = sql_fetch($sql);
if(!$mb['id'])
    alert('존재하지 않는 회원입니다.');
