...
// (2021-01-29) 모바일 복지 상점코드 오류 (복지포인트 창이 안뜸)
include_once(TB_MSHOP_PATH.'/settle_kcp2.inc.php'); // 환경설정 파일 include
?>
<?php
/* = -------------------------------------------------------------------------- = */
...
$js_return_url = TB_MSHOP_URL.'/orderkcp2.php?od_id='.$ordr_idxx;
?>
...
if($enc_data != '' && $enc_info != '' && $tran_cd != '') {
    ...
    $sql = " select * from shop_order_data where od_id = '$ordr_idxx' ";
    $row = sql_fetch($sql);
...
<!-- 사이트 코드 -->
<input type="text" name="site_cd"      value="<?php echo $g_conf_site_cd; ?>">
 <!-- 사이트 이름 -->
...
<input type="hidden" name="Ret_URL"      value="<?php echo TB_MSHOP_URL; ?>/kcp/order_approval_form2.php">
<!-- 인증시 필요한 파라미터(변경불가)-->
