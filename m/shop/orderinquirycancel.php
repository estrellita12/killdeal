   ...
   if($stotal['usepoint2'] > 0) //기본금 사용 존재
   {
		 ...
		 for($i=0; $row=sql_fetch_array($result); $i++)
         {
	           ...
	           if($row['magam_uresult'] == 'Y')//포인트사용 마감호출 O ->포인트 취소마감호출->현금영수증 신청여부 체크
			   {
				      ...
				      if($i == 0)//한번만 실행
				      {
				         ...
				         if($row['cash_uresult'] == 'Y')//현금영수증 신청 성공여부
				         {
						     ...
						     if($taxsave_yes =='Y' || $taxsave_yes == 'S')//현금영수증 존재
			                 {
						           ...
						           $cashurl = "https://mgift.e-hyundai.com/hb2efront_new/openAPICashReceipt.do?".http_build_query($hdata_cach);
				    ...
				    $url2 = "https://mgift.e-hyundai.com/hb2efront_new/pointOpenAPIMagam.do?".http_build_query($hdata2);
...
// 2021-08-09
    ...
    if($i_usepoint > 0) //적립금 사용 존재
    {
        ...
        $res_dec = golfrock_point('point_cancel', $member_id[1], $member['name'] , $od_id,'골프용품', $i_usepoint );
    }
}
