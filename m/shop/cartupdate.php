...
if($act == "buy")
{
    ...
    if($is_member){ // 회원인 경우
        ...
        if($pt_id == 'golfu'){
            ...
            goto_url('https://www.golfu.net/Mobile/Main/Default.aspx?strPrevUrl=http://shopping.golfu.net/m/shop/orderform.php');
        }else {
...
if($sw_direct == '1') {
    if($is_member)
        ...
        $referer = $_SERVER["HTTP_REFERER"];
        $gs_id = explode('=', $referer);
        goto_url(TB_MBBS_URL.'/login.php?url='.urlencode('/shop/orderform.php?gs_id='.$gs_id[1]));
    }
} else if ($sw_direct == '0') {
    //alert();
    goto_url(TB_MSHOP_URL."/cart.php");
