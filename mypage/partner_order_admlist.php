...
//20200319 취소제외 총 주문금액 
$sql_search2 = " where dan not in (0,6,7,9,10,11) and pt_id = '{$member['id']}' ";
if($fr_date && $to_date)
    $sql_search2 .= " and left({$sel_field},10) between '$fr_date' and '$to_date' ";
else if($fr_date && !$to_date)
    $sql_search2 .= " and left({$sel_field},10) between '$fr_date' and '$fr_date' ";
else if(!$fr_date && $to_date)
    $sql_search2 .= " and left({$sel_field},10) between '$to_date' and '$to_date' ";

$cancel_tot_orderprice = 0; //취소,환불 제외 총 주문액
$sql = " select od_id {$sql_common} {$sql_search2} {$sql_group} {$sql_order} ";
$res = sql_query($sql);
while($row=sql_fetch_array($res)) {
	$amount = get_order_spay($row['od_id']);
	$cancel_tot_orderprice += $amount['buyprice'];
}

//20200319 취소제외 총 주문금액 판매수수료

$commission_tot_orderprice = 0;
$pt_id = $member['id'];

$sql_search2 = '';

if($fr_date && $to_date)
    $sql_search2 .= " and left(pp_datetime,10) between '$fr_date' and '$to_date' ";
else if($fr_date && !$to_date)
    $sql_search2 .= " and left(pp_datetime,10) between '$fr_date' and '$fr_date' ";
else if(!$fr_date && $to_date)
    $sql_search2 .= " and left(pp_datetime,10) between '$to_date' and '$to_date' ";

$sql = " SELECT sum(pp_pay) as sum_pay FROM shop_partner_pay WHERE mb_id = '$pt_id' {$sql_search2}";
//echo $sql_search;
$res = sql_query($sql);

while($row=sql_fetch_array($res)) {
	$commission_tot_orderprice = $row['sum_pay'];
}
...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td>
			<select name="sfl">
                ...
                <?php echo option_selected('cellphone', $sfl, '주문자핸드폰'); ?>
	...
	<tr>
		...
		<td>
			...
			<?php echo radio_checked('od_settle_case', $od_settle_case, '복지카드', '복지카드'); ?>
			<?php echo radio_checked('od_settle_case', $od_settle_case, '기본금', '기본금'); ?>
		</td>
		</td>
	</tr>
	<tr>
...
<div class="local_ov mart30">
    ...
    <!--
	<strong class="ov_a">취소제외 총 주문금액 : <?php echo number_format($cancel_tot_orderprice); ?>원</strong>
	<strong class="ov_a">취소제외 총 주문금액 판매수수료 : <?php echo number_format($commission_tot_orderprice); ?>원</strong>
    -->
</div>
...
<div class="tbl_head01">
	...
	<tr class="<?php echo $bg; ?>">
        ...
        <td rowspan="<?php echo $rowspan; ?>">
            <a href="./pop_orderform.php?od_id=<?php echo $row['od_id']; ?>" onclick="win_open(this,'pop_orderform','1200','800','yes');return false;" class="fc_197"><?php echo $row['od_id']; ?></a>
		...
		<td rowspan="<?php echo $rowspan; ?>"><?php echo preg_replace("/[ #\&\+\%@=\/\\\:;,\.'\"\^`~\_|\!\?\*$#<>()\[\]\{\}]/i", "", $row2['cellphone']); ?></td>
		...
		<td rowspan="<?php echo $rowspan; ?>"><?php echo $row2['rcent_time']; ?></td>
		<?php } ?>
	<?php
		}
	}
