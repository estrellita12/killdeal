...
<div class="tbl_frm02">
	...
	<tr>
		...
		<td>
			...
			<?php
            // (2020-12-22) 가맹점에서 메인 배너 1개만 등록 가능 
            /*
			for($i=0; $i<count($position); $i++)
				echo option_selected($position[$i][0], $bn['bn_code'], $position[$i][3]); 
            */
			echo option_selected($position[0][0], $bn['bn_code'], $position[0][3]); 
			?>
			</select>
			<?php 
            //echo help('[고정] 같은 위치에 고정되어 노출되며 2개이상 등록시 랜덤으로 노출됩니다.<br>[연속] 2개이상 등록시 세로로 연속하여 노출됩니다.<br>[롤링] 2개이상 등록시 슬라이드형식으로 롤링되며 노출됩니다.'); 
            ?>
		</td>
	</tr>
	<tr>
