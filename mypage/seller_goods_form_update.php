...
$value['seller_comm']	= conv_number($_POST['seller_comm']); //공급사 수수료
...
$value['brand_uid']		= $_POST['brand_nm']; //브랜드주키
...
// 2022-01-06
/*
if($mb_id=='majorgolf' || $mb_id == 'AP-100002') $config['seller_reg_auto'] = 0;
else $config['seller_reg_auto'] = 3;

if($mb_id=='majorgolf' || $mb_id == 'AP-100002') $config['seller_mod_auto'] = 0;
else $config['seller_mod_auto'] = 3;
*/
...
if($w == "") {
    ...
    // (2021-12-24)
    $rows = goods_price_sel($gs_id);
    $normal_price = $rows['normal_price'];
    $supply_price = $rows['supply_price'];
    ...
    $memo = $rows['memo'];
    /*
    $arr[0] = "opt_id : ".implode(",",$_POST['opt_id']);
    $arr[1] = "price : ".implode(",",$_POST['opt_price']);
    $arr[2] = "stock_qty : ".implode(",",$_POST['opt_stock_qty']);
    $arr[3] = "noti_qty : ".implode(",",$_POST['opt_noti_qty']);
    $arr[4] = "use : ".implode(",",$_POST['opt_use']);
    $memo = implode("/",$arr);
    */
    ...
    goods_price_update($gs_id, "(update)".$mb_id, $normal_price, $supply_price, $goods_price, $gname,$memo,$isopen);
}
