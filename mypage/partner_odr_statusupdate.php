...
if($_POST['act_button'] == '결제완료') {
    ...
    // (2021-03-04)
    order_change_update($od_id, $od_no, $member['id'], $gw_status[$od['dan']], $gw_status[2] , ''  );
...
else if($_POST['act_button'] == "주문취소")
{
	...
	while($row=sql_fetch_array($res)) {
        // (2021-03-04)
        order_change_update($od_id, $od_no, $member['id'], $gw_status[$od['dan']], $gw_status[6] , ''  );
	...
	for($i=0; $i<$chk_count; $i++)
	{
		...
		switch($change_status) {
			...
			//case '6': // 취소
				...
				//break;
			case '7': // 반품
				if($current_status != 5 && $current_status != 11) continue;
