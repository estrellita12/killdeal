...
<form name="buyform" id="buyform" method="post" action="https://mjg.kr/shop/orderformupdate_benepia.php" onsubmit="return fbuyform_submit(this);" autocomplete="off">
    <input type="hidden" name="pt_memcorp_cd"     value="5555"/> <!--베네피아 사이트 코드-->
    <input type="hidden" name="req_tx"      value="pay">
    <input type="hidden" name="site_cd"     value="AJYU8"/>
    <input type="hidden" name="pay_method"  value="POINT"/>
    <input type="hidden" name="pt_issue"    value="SCWB"/>
...
?>
                <tr>
                    <td class="tac">
                        ...
                        <input type="hidden" name="gs_name" value="<?php echo $it_name; //제품 하나만 입력 한다. ?>">
...
<?php
    if($is_member && $config['usepoint_yes']  && $pt_id != 'golf') { ?>
                        ...
                        <!-- 베네피아 포인트 추가 -->
                        <tr class="benepia" style="position:relative;z-index:1;">
                            <th scope="row">베네피아포인트</th>
                            <td style="position:relative"><label><input type="text" readonly name="point" value="0" class="frm_input" size="12"></label><button type="button" class="btn_small blue point_search">포인트조회</button>
                                <table style="display:none;position:absolute;left:0;border:1px solid gray;width:250px;background:#fff;">
                                    <tbody >
                                    <tr style="padding:10px">
                                        <th scope="row" style="font-size:12px;padding:10px">베네피아아이디</th>
                                        <td><label><input type="text" name="pt_idno" value="" class="frm_input" size="12"></label></td>
                                    </tr>
                                    <tr >
                                        <th scope="row" style="font-size:12px;padding:10px">베네피아패스워드</th>
                                        <td><label><input type="password" name="pt_pwd" value="" class="frm_input" size="12"></label><button type="button" class="btn_small blue query">확인</button></td>
                                    </tr>
                                    <tr style="padding:10px">
                                        <th scope="row" style="font-size:12px;padding:10px">전체포인트</th>
                                        <td><label><input type="text" readonly name="point_all" value=" " class="frm_input" style="border:0;" size="12"></label></td>
                                    </tr>
                                    <tr style="padding:10px">
                                        <th scope="row" style="font-size:12px;padding:10px">사용할포인트</th>
                                        <td><label><input type="number" name="point_use" value=" " min="1" max="<?php echo $tot_price;?>" class="frm_input" size="12"></label><button type="button" class="btn_small blue use">사용하기</button></td>
                                    </tr>
                        ...
                        <script>
                            $('.point_search').click(function(){
                                $('.benepia table').toggle();
                            });



                            $('.query').click(function(){
                                var id = $('[name=pt_idno]').val();
                                var pw = $('[name=pt_pwd]').val();
                                $.ajax({
                                        async : false,
                                        type: "POST",
                                        url: 'https://mjg.kr/theme/benepia/kcp/kcp_api_page.php',
                                        data: {pt_idno:id, pt_pwd:pw, req_tx: 'query'},
                                        ...
                                        success: function(data) {
                                            ...
                                            $('[name=point_all]').val(data.rsv_pnt);
                                            //$('[name=point_use]').val(data.rsv_pnt);

                                            $("[name=point_use]").keydown(function(e){
                                                if($(this).val() > parseInt($('[name=point_all]').val())){
                                                    alert('사용할 포인트가 초과하였습니다. ');
                                                    $(this).val('');return false;
                                                }
                                                if($(this).val() > parseInt($(this).attr('max'))){
                                                    alert('금액전체를 포인트로 사용하시겠습니까?');
                                                    $('.use').trigger('click');
                                                }
                                            ...
                                            $('.use').click(function(){
                                                if(parseInt($('[name=point_all]').val()) < parseInt($('[name=point_use]').val())){
                                                    alert('사용할 포인트가 초과하였습니다. ');return false;
                                                }
                                                if(parseInt($('[name=point_use]').val()) > parseInt($('[name=point_use]').attr('max'))){
                                                    $('[name=point]').val($('[name=point_use]').attr('max'));
                                                    $('[name=point_use]').val($('[name=point_use]').attr('max'));
                                                } else {
                                                    $('[name=point]').val($('[name=point_use]').val());
                                                }
    ...
    function calculate_order_price() {
        ...
        $('[name=use_point]').val($('[name=point]').val());
        ...
        // 박스 금액 수정
        $("#h_point").html('<span class="spr">'+number_format(String(mb_point))+'<span>원</span></span>');
        $("#tot_price3").html('<span class="spr">'+number_format(String(tot_price))+'<span>원</span></span>');
