...
<!-- 		</div> -->
	...
	<div class="vi_txt_bx">
		...
		<div class="price_bx">
			...
			<?php } ?>
			<dl>
				<dt class="padt5">할인코드</dt>
				<dd class="pr_code"><?php echo $pt_id; ?></dd>
			</dl>			
			<dl>
...
echo
    '<script>
            const over_estimated_pr = $(".mpr").eq(0).text();
            $(".mpr").eq(0).text(`${number_format(Math.ceil(Number(over_estimated_pr.replaceAll(",","").replaceAll("원","")) / 0.9 / 100) * 100)}원`);
			$(".mpr").css("font-weight","normal");
			$(".wish_btn").css("display","none");
			$(".mpr").append(`<span style="font-size : 20px;"> --> </span>`);
			$(".mpr").append(`<span style="font-size : 20px; color:#fd0000;">${over_estimated_pr} </span>`);
			$(".mpr").append(`<span style="; font-size : 15px; background:black; color:white;">10% 추가할인★</span>`);
    </script>';
?>