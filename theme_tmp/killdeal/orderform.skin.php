		...
		$(document).ready(function(){
			...
			$.ajax({
		              ...
		              success : function(result){  
							  ...
							  $("#hwelpoint").html(return_point) ;
							  document.buyform.mb_point2.value = return_point;
							  });							 
...
<!-- 현대복지몰 포인트 -->
<input type="text" name="mb_point2">
...
<input type="text" name="org_price" value="<?php echo $tot_price; ?>">
...
<section id="sod_fin_pay">
	...
	<div class="tbl_frm01 tbl_wrap">
		...
		<tr>
			...
			<td class="td_label">
				...
				if($is_member && $pt_id == "golf" ) {
					echo '<input type="radio" name="paymethod" id="paymethod_point2" value="복지카드" onclick="calculate_paymethod(this.value);"> <label for="paymethod_point2">복지포인트결제</label>'.PHP_EOL;
				}
		...
		<?php } ?>
	  	...
	  	<?php
		//현대리바트 포인트_20190603
		//위의 포인트 결제부분은 필요없을것으로 보인다.
		if($is_member && $pt_id == 'golf') { ?>
		<tr>
			<th scope="row">현대복지포인트결제</th>
			<td>
				<input type="text" id="use_point2" name="use_point2" value="0" class="frm_input" size="12" onkeyup="calculate_temp_point2(this.value);this.value=number_format(this.value);" style="font-weight:bold;"> 원 보유포인트 : 
				<span id="hwelpoint"></span>
			</td>
		</tr>
		<?php } ?>
...
function calculate_order_price() {
	...
	?>
	mb_point2   = parseInt($("input[name=use_point2]").val().replace(/[^0-9]/g, "")); //복지포인트결제
...
<?php } ?>
	...
	var mb_point2	= parseInt(f.mb_point2.value);//리바트
...
function calculate_paymethod(type) {
	...
	//현대리바트
	if( type == '복지카드' && mb_point2 < tot_price ) {
		alert('포인트 잔액이 부족합니다.');
