...
<head>
  ...
  <style>
    ::placeholder{color:#9d9d9d;text-align:center;}
    #event_wrap{position:relative;left:0;top:0;width:1200px;margin:0 auto;}
    #input_form_wrap{position:absolute;height:900px;left:50%;bottom:11%;width:70%;background:#fff;transform:translateX(-50%);padding:30px;border-radius:15px;text-align:center;border:1px solid #000;}
    #input_form_wrap input{width:100%;height:80px;font-size:25px;font-weight:600;margin-top:20px;text-align:center;}
    #input_form_wrap #agree_box{height:200px;width:100%;margin-top:20px;}
    #input_form_wrap input#agree_chk{height:20px;width:30px;vertical-align:sub;}
    #input_form_wrap input:focus{outline:none;}
    #event_btn_wrap{margin-top:20px;}
    .agree_notice{font-size:16px;color:#999;margin:5px 0 0;letter-spacing:-0.075em;}
    #eventBtn,#woongmoBtn,#inzngBtn{width:100%;height:60px;}
    #eventBtn:focus,#woongmoBtn:focus,#inzngBtn:focus{outline:none;}
	#inzngBtn,#eventBtn,#woongmoBtn{background:#00f200;color:#000;border:none;font-weight:700;transition:all 0.3s;font-size:28px;box-shadow:3px 3px 0 black;}
    #inzngBtn:hover,#eventBtn:hover{background:#00ea00;color:#000;border:none;font-weight:700;}
	#demo{position:absolute;left:0;top:50%;}
  </style>
</head>
<body>
  <div id="event_wrap">
    <div id="event_bg">
      <img src="/img/20200807_event.jpg" alt="이벤트이미지" usemap="#event_map">
	  <map name="event_map"><area shape="rect" coords="393,4209,785,4306" href="https://www.youtube.com/channel/UC1vCF_oF13DLh7l5TAQGHMQ" alt="랜선골프유튜브" target="_blank"></map>
    </div>
    <div id="input_form_wrap">
      <h2 style="font-size:34px;">정보 입력</h2>
      <p style="font-size:20px;color:red;font-weight:600;letter-spacing:-0.075em;">이벤트 참여 및 당첨 결과 안내를 위해 반드시 입력해주세요!</p>
...
* 위탁기간: 3개월
        ...
        <div id="agree_wrap">
          <label style="font-size:16px;">개인정보 수집 및 이용 동의 (필수)
...
<!--       <button type="button" style="margin-top:20px;" id="eventBtn" onclick="return eventAjax()">인증번호받기</button> -->
      <div id="event_btn_wrap">
<!--         <button type="button" style="display:none;margin:0 auto;" id="inzngBtn" onclick="inzngGo()">인증하기</button> -->
        <button type="button" style="display:block;" id="woongmoBtn" onclick="woongmo()">응모하기</button>
      </div>
    </div>
<!-- 	  <div id="demo"></div> -->
  </div>
