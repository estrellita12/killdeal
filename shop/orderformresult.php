...
{
    // (2022-08-05) 결제 누락 방지를 위해 dan값 추가
    $tmp_dan = "update shop_order set dan = '1' where od_id = '$od_id' ";
    sql_query($tmp_dan,false);
    ...
    // (2021-01-08) 결제 누락 방지를 위해 dan값 추가
    $tmp_dan = "update shop_order set dan = '2' where od_id = '$od_id' ";
    sql_query($tmp_dan,false);
...
if($tno) {
    if((int)$i_price !== (int)$pg_price) {
        ...
        $sql = " update shop_order set dan = '6' , cancel_date = '".TB_TIME_YMDHIS."', od_mod_history='주문금액과 결제금액 불일치' where od_id = '$od_id' ";
        sql_query($sql);
...
if($result) {
    ...
    // (2020-12-21) 오류 발생시 주문 취소 
    $sql = " update shop_order set dan = '6' , cancel_date = '".TB_TIME_YMDHIS."', od_mod_history='주문정보 저장 오류' where od_id = '$od_id' ";
    sql_query($sql);
