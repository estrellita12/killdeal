...
?>
<div class="vi_txt_li">
	<dl class="cart_opt_txt">
		<dt class="opt_select_txt">주문옵션<span>(아래옵션은 필수선택 옵션입니다)</span></dt>
	</dl>
	<?php echo $option_1; ?>
</div>
<?php } ?>
...
<div id="option_set_list">
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if(!$row['io_type'])
			$io_price = display_price($row['io_price'] + $gs['goods_price']);
		else
			$io_price = display_price($row['io_price']);
		...
		<li class="sit_<?php echo $cls; ?>_list vi_txt_li">
			<dl>
				...
				<dd class="op_vi_txt">
					...
					<label>
						<input type="text" name="ct_qty[<?php echo $gs_id; ?>][]" value="<?php echo $row['ct_qty']; ?>" class="inp_opt cart_inp" size="2">
					</label>
...
<div class="btn_confirm">
	...
	<button type="button" id="mod_option_close" class="btn_medium red">취소</button>
</div>
</form>
