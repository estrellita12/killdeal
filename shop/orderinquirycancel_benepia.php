...
$site_cd            = 'AJYU8'; // 사이트코드
// 인증서 정보(직렬화)
//$kcp_cert_info  = file_get_contents('../shop/kcp/key/KCP_AUTH_AJXCC_CERT.pem');
...
//
$cancel_target_data = $site_cd."^".$od['od_tno']."^STSC";
// 결제 취소 (cancel) = site_cd^KCP거래번호^취소유형, STSC는 취소
// 사인데이타 필요

$key_data = file_get_contents('../shop/kcp/key/KCP_AUTH_AJYU8_PRIKEY.pem');
// 개인키 경로 ("splPrikeyPKCS8.pem" 은 테스트용 개인키)
$pri_key = openssl_pkey_get_private($key_data, 'major2024@@');
// 개인키 비밀번호
...
$pay_method           = 'POINT';
$amount               = $_POST[ "amount"  ] = $_POST["point"];     // 총결제금액
$currency             = "";                      // 화폐단위
...
$good_name            = $_POST[ "good_name"  ] = '테스트';  // 상품명
...
//취소 전문
$mod_type       = $_POST["mod_type" ] = 'STSC'; // 변경유형
$tno            = $_POST["od_tno" ] = $od['od_tno'];      // NHN KCP 거래번호
$mod_desc       = $_POST["mod_desc" ] ='dd'; // 변경 사유
