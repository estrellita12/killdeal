...
*/
  ...
  //***1.시간단위호출 od_time >= 현재-1 and od_time < 현재시간
  //****중요 :마감취소호출시 취소한 날짜를 던져야 한다.
  $sql = " select * from shop_order where use_point2 > 0 and  pt_id = 'golf' and  od_id in ('23100212573366')  order by index_no desc ";
  ...
  for($i=0; $row=sql_fetch_array($result); $i++)
  {
        ...
        if($order_type_code == 10){
            $mproc_code = base64_encode("10");//**포인트 사용(10:사용 ,20: 포인트 취소)
        }else{
            $mproc_code = base64_encode("20");//**포인트 사용(10:사용 ,20: 포인트 취소)
        }		
		...
		$shop_no = base64_encode($row['shop_no']);//상점번호
		$shop_no2 = exec("/usr/local/java/bin/java -classpath .:hcdesutil.jar BB ".$shop_no);

		$shopevent_no = base64_encode($row['shopevent_no']);//행사번호
		$shopevent_no2 = exec("/usr/local/java/bin/java -classpath .:hcdesutil.jar BB ".$shopevent_no);
