...
$sql = "select * from shop_goods where shop_state = 0 and index_no in ({$sql_gcode})  limit 4";
$res = sql_query($sql);

$goods_info = array();
for($i=0; $row=sql_fetch_array($res); $i++) {
    $discount_rate = floor(($row['normal_price']-$row['goods_price'])/$row['normal_price']*100);
    $good = array("img" => 'https://shop.golfrock.co.kr/data/goods/'.$row['simg1'], "name" => $row['gname'], "normal_price" => $row['normal_price'],
