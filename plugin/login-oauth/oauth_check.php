...
if($mb['id']) { // 가입된 회원이면
    ...
    // (2021-04-07)
    $sql = " update shop_member set login_sum = login_sum + 1, today_login = '".TB_TIME_YMDHIS."', login_ip = '{$_SERVER['REMOTE_ADDR']}' where id = '{$mb['id']}' ";
    sql_query($sql);
    ...
    // (2021-04-26)
    if($msg = exist_pt_mb_email($pt_id, $mb_email, $mb_id))   alert($msg,"close");
    ...
    $value['cellphone']		= $mb_phone; //전화번호 2021-04-09
