...
//20200427 매거진 비밀글 관리자에서 만 노출
if($_SESSION["ss_mb_id"] != 'admin'){
    $sql_search = " where btype = '2' {$sql_search2} and issecret = 'N' ";
}else{
...
<ul class="webzine">
	...
	for($i=0; $row=sql_fetch_array($rst); $i++) {
		...
		if((TB_SERVER_TIME-$row['wdate']) < (60*60*24))
			$bo_newicon = "&nbsp;<img src='".$bo_img_url."/img/iconY.gif' alt=\"new\">";
		else
			$bo_newicon = "";
	...
	<dl>
		...
		<dd><?php echo $row['writer_s']; ?><span><?php echo $bo_wdate; ?></span><!--20191128 조회수 주석 <span>조회 : <?php //echo $row['readcount']; ?></span>--></dd>
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$thumb = get_list_thumbnail($boardid, $row, 120, 0);
		// 20191017 기존 썸네일 코드 예외처리 새로운 썸네일 코드 추가
		/*
		if(!$thumb['src']) {
			$thumb['src'] = TB_IMG_URL.'/noimage.gif';
		}
		*/
        $thumb['src'] = "http://i.ytimg.com/vi/{$row['fileurl1']}/mqdefault.jpg";
	?>
	<li>
		<a href="<?php echo $bo_href; ?>">
		<img class="magazine_img" src="<?php echo $thumb['src']; ?>" alt="<?php echo $thumb['alt']; ?>">
		<p class="media_cont_tit">
			<?php echo $bo_subject; ?><?php if($row['issecret'] == 'Y') { ?>&nbsp;<img src="<?php echo $bo_img_url; ?>/img/icon_secret.gif"><?php } ?><?php echo $bo_newicon; ?>
		</p>
        <!-- (2021-03-16) -->
		<!-- <span class="cont_date"><?php echo $row['writer_s']; ?> <span><?php echo $bo_wdate; ?></span> <span>조회 : <?php //echo $row['readcount']; ?></span>-->
		<?php if(is_admin()) { ?>
			<?php if($row['main_use'] == '1') { ?><span style="color:red;">메인노출 </span><?php } ?>
		<?php } ?>
		</span>
<!-- 	20200630내용 주석	<dd class="bo_txt"><?php echo strip_tags($row['memo']);?></dd> -->
		</a>
		<?php if(is_admin()) { ?>
		<div class="bo_chk"><label><input type="checkbox" name="OrderNum[]" value="<?php echo $row['index_no']; ?>"></label></div>
		<?php } ?>
	</li>
	<?php
		$run++;
