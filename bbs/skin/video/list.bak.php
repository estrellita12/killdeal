...
if($_SESSION["ss_mb_id"] != 'admin')
{
    $sql = " select * $sql_common $sql_search and issecret = 'N' $sql_order limit $from_record, $rows ";
}
else
{
...
<ul class="webzine">
	...
	<dl>
		...
		<dd><?php echo $row['writer_s']; ?><span><?php echo $bo_wdate; ?></span><span>조회 : <?php echo $row['readcount']; ?></span>
		<?php if(is_admin()) { ?>
		...
		<?php if(is_admin()) { ?>
		<dd class="bo_chk"><label><input type="checkbox" name="OrderNum[]" value="<?php echo $row['index_no']; ?>"></label></dd>
		<?php } ?>
	</dl>
	<?php
		$run++;
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$thumb = get_list_thumbnail($boardid, $row, 120, 0);
		/* 20191017 기존 코드 예외 처리 유튜브 썸네일 대체
		if(!$thumb['src']) {
			$thumb['src'] = TB_IMG_URL.'/noimage.gif';
		}
		*/
		...
		$thumb['src'] = merge_thumnail_url($tag[0]);
	?>
	<li>
	<?php if($member['id'] == 'admin') { ?>
	<a href="<?php echo $bo_href; ?>">
	<?php } else { ?>
	<a href="https://www.youtube.com/watch?v=<?php echo $tag[0]; ?>" target="_blank">
	<?php } ?>
		...
		<span class="cont_date"><span><?php echo $bo_wdate; ?></span>
		<?php if(is_admin()) { ?>
			<?php if($row['main_use'] == '1') { ?><span style="color:red;">메인노출 </span><?php } ?>
		<?php } ?>
		</dd>
