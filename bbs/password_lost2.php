...
// (2021-04-26)
$sql = " select count(*) as cnt from shop_member where email = '$email' and pt_id = '$pt_id'";
$row = sql_fetch($sql);
if($row['cnt'] > 1)
    //alert('동일한 메일주소가 2개 이상 존재합니다.\\n\\n관리자에게 문의하여 주십시오.');
    alert('고객님, 아이디찾기/비밀번호 변경을 위해서는\\n\\n관리자에게 문의하여 주십시오.\\n\\n불편을 드려 죄송합니다');

// (2021-04-26)
$sql = " select index_no,sns_id, id, name, email from shop_member where email = '$email' and pt_id = '$pt_id' ";
$mb = sql_fetch($sql);
if(!$mb['id'])
    alert('존재하지 않는 회원입니다.');

if($mb['sns_id']){
    if( substr($mb['id'],0,1) == "k"){
        alert_close('카카오 아이디로 로그인한 회원입니다.');
    }else{
        alert_close('네이버 아이디로 로그인한 회원입니다.');
    }
}
// 임시비밀번호 발급
