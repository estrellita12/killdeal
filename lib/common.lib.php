...
function get_sideview($mb_id, $name)
{
	...
	$str2.= "<a href=\"".TB_ADMIN_URL."/pop_memberorder.php?mb_id={$mb_id}\" onclick=\"win_open(this,'win_member','1200','600','yes');return false;\">주문내역</a>\n";

    // (2021-02-05) 관리자 페이지에서 가맹점 정보 수정
    if(is_partner($mb_id))
        $str2.= "<a href=\"".TB_ADMIN_URL."/pop_partner_info.php?mb_id={$mb_id}\" onclick=\"win_open(this,'win_parnter','1200','600','yes');return false;\">가맹점정보수정</a>\n";
...
function display_logo($filed='basic_logo')
{
	...
	if(is_file($file) && $row[$filed]) {
		...
		return '<a href="'.TB_URL.'"><img src="'.$file.'" alt="로고" ></a>';
	} else {
		return '';
	}
}

function display_footer_logo($filed='basic_logo')
{
	global $pt_id;
...
// get_listtype_skin('영역', '이미지가로', '이미지세로', '총 출력수', '추가 class') 인스타
function get_listtype_skin2($type, $width, $height, $rows, $li_css='')
{
	global $pt_id;
	...
	for($i=0; $row=sql_fetch_array($result); $i++)
	{
		...
		$str .= "<li>\n";
			...
			$str .= "<dl>\n";
				...
				$str .= "<dd class=\"pname\">{$it_name}</dd>\n";
				//$str .= "<dd class=\"price\">{$sale}&nbsp;{$it_sprice}{$it_price}</dd>\n";
				//$str .= "<dd class=\"price\">$sale&nbsp;<span class=\"price_box\">{$it_price}<br>{$it_sprice}</span></dd>\n";
				$str .= "<dd class=\"price\">$sale&nbsp;<span class=\"price_box\">{$it_sprice}<span class=\"recommendation\">특별할인★</span>{$it_price}</span></dd>\n";
				if( !$is_uncase && ($row['gpoint'] || $is_free_baesong || $is_free_baesong2) ) {
					$str .= "<dd class=\"petc\">\n";
					if($row['gpoint'])
						$str .= "<span class=\"fbx_small fbx_bg6\">{$it_point} 적립</span>\n";
...
// get_listtype_skin('영역', '이미지가로', '이미지세로', '총 출력수', '추가 class') 캐디테마
function get_listtype_skin3($type, $width, $height, $rows, $li_css='')
{
	global $pt_id;
	...
	for($i=0; $row=sql_fetch_array($result); $i++)
	{
		...
		$str .= "<li>\n";
			...
			$str .= "<dl>\n";
				...
				$str .= "<dd class=\"pname\">{$it_name}</dd>\n";
				//$str .= "<dd class=\"price\">{$sale}&nbsp;{$it_sprice}{$it_price}</dd>\n";
				$str .= "<dd class=\"price\">$sale&nbsp;<span class=\"price_box\">{$it_price}{$it_sprice}</span></dd>\n";
				if( !$is_uncase && ($row['gpoint'] || $is_free_baesong || $is_free_baesong2) ) {
					$str .= "<dd class=\"petc\">\n";
					if($row['gpoint'])
						$str .= "<span class=\"fbx_small fbx_bg6\">{$it_point} 적립</span>\n";
...
function get_cart_listtype_skin($type, $width, $height, $rows, $li_css='')
{
	global $pt_id;
...
function get_listtype_cate($list_best, $width, $height)
{
	...
	for($i=0; $i<count($list_best); $i++) {
		...
		for($g=0; $g<count($list_code); $g++) {
			...
			//$row = sql_fetch(" select * from shop_goods where index_no = '$gcode' and shop_state=0 "); // (2021-02-24)
            $row = sql_fetch(" select * from shop_goods where index_no = '$gcode' "); // (2022-02-16) 진열 대기 상품 보이도록 변경
			...
			$str .= "<li class=\"{$gcode}\">\n";
				$str .= "<div>\n";
					$str .= "<a href=\"{$it_href}\">\n";
                        if( strpos($it_price,"품절") || strpos($it_price,"중지")){
                        $str .= "<div class=\"soldout_layer\"></div>\n";
                        }
						$str .= "<dl>\n";
							$str .= "<dt>{$it_image}</dt>\n";
							$str .= "<dd class=\"pname\">{$it_name}</dd>\n";
...
// 인플루언서몰 전용 인기상품 리스트
function get_listtype_cate2($list_best, $width, $height)
{
	$mod = 8;
	$ul_str = '';
...
// 캐디 테마
function get_listtype_cate3($list_best, $width, $height, $mod=null)
{
	$mod = !empty($mod) ? $mod : 10;
	$ul_str = '';
...
// 게시판 리스트 가져오기 (#1, #2 2023-02-16 공지사항 비밀글은 관리자만 보이도록 수정)
function board_latest($boardid, $len, $rows, $pt_id)
{
	...
	if($default['de_board_wr_use']) {
        ...
        if(!is_admin()) $sql_where .= "and issecret = 'N' "; // #1
	}
    if(!is_admin()) $sql_where .= "where issecret = 'N' "; // #2
...
// 비디오게시판 리스트 가져오기
function board_video_latest($boardid, $len, $rows, $pt_id)
{
	global $default;
	...
	for($i=0;$row=sql_fetch_array($res);$i++){
		...
		// $str .= "<div>\n";
		$str .= "	<a href=\"https://www.youtube.com/channel/UCUXDKiEsZH9Kcad-hksAK7g\" target=_blank class=\"hong_link_btn\">홍인규골프채널바로가기</a>\n";
		$str .= "	<a href=\"https://www.youtube.com/channel/UC1vCF_oF13DLh7l5TAQGHMQ\" target=_blank class=\"video_link_btn\">마마골프채널바로가기</a>\n";
...
// 골프매거진 리스트 가져오기 캐디테마
function board_video_latest3($boardid, $len, $rows, $pt_id){
    global $default;
...
//20200729 골프비디오 썸네일가져오기
function video_thumb($boardid, $len, $rows, $pt_id){
	global $default;
	...
	for($i=0;$row=sql_fetch_array($res);$i++){
		...
		$str .= " <img src=\"img/mini_ch_art.png\" class=\"mini_img\">\n";
		$str .= " <img src=$url_th style=\"width:640px;\">\n";
		$str .= " </a>\n";
		$str .= " </div>\n";
	}
...
// 골프매거진 리스트 가져오기
function board_mgz_latest($boardid, $len, $rows, $pt_id)
{
	global $default;
...
// 골프매거진 리스트 가져오기 캐디테마
function board_mgz_latest2($boardid, $len, $rows, $pt_id)
{
	global $default;
...
function review_cnt()
{
	$sql = " select count(*) as cnt from shop_goods_review ";
	$row = sql_fetch($sql);
	return (int)$row['cnt'];
}

// 2024-02-15
function review_cnt2()
{
    $sql = " select count(*) as cnt from ( ( select index_no from shop_goods_review ) union ( select index_no from shop_goods_review_2 )   ) as c";
    $row = sql_fetch($sql);
    return (int)$row['cnt'];
}
...
function excel_order_list($row, $amount)
{
    ...
    // (2021-01-19) 현대리바트 기본금 사용 : 저장은 포인트로, 보여지는것은 기본금으로
    if($row['pt_id']=='golf' && $od_paytype=='포인트'){
        $od_paytype = '기본금';
    }
    
    if($amount['usepoint2'] > 0 && $row['paymethod'] != '포인트' && $row['paymethod']!='기본금')
		$od_paytype.= '+기본금';
...
function get_order_list($row, $amount, $baesong_search='')
{
	...
	if($row['paymethod']) {
		...
		if($row['paymethod'] == '간편결제') {
            ...
            $disp_paytype = "PG간편결제"; // 2024-01-29
		}
	} else {
		$disp_paytype = '결제수단없음';
	}
    ...
    // (2021-01-19) 현대리바트 기본금 사용 : 저장은 포인트로, 보여지는것은 기본금으로
    if($row['pt_id']=='golf' && $disp_paytype=='포인트'){
        $disp_paytype = '기본금';
    }
    
    if($amount['usepoint2'] > 0 && $row['paymethod'] != '포인트' && $row['paymethod']!='기본금')
        $disp_paytype.= '<span class="list_point">기본금</span>';
...
function admin_gs_jaego_bujog($add_query='')
{
	...
	$sql = " select count(*) as cnt from shop_goods where stock_qty <= 1 {$add_query} ";
	$row = sql_fetch($sql);
...
{
    // (2021-03-08)
	//$sql = " select count(*) as cnt from shop_goods_option a left join shop_goods b on (a.gs_id=b.index_no) where a.io_use = 1 and a.io_noti_qty <> '999999999' and a.io_stock_qty <= a.io_noti_qty {$add_query} ";
	$sql = " select count(*) as cnt from shop_goods_option a left join shop_goods b on (a.gs_id=b.index_no) where a.io_use = 1 and a.io_noti_qty <> '999999999' and a.io_stock_qty <= 1 {$add_query} ";
	$row = sql_fetch($sql);
...
function get_price($gs_id, $msg='<span>원</span>')
{
	...
	if((!$is_member && $pt_id == 'honggolf')||(!$is_member && $pt_id == 'maniamall')){ //2020623 마니아몰,홍골프 비회원 가격비공개
		$str = "<span class=\"memopen\">회원공개</span>";
	}
	return $str;
}
...
function get_buy_button($msg, $gs_id)
{
	...
	for($i=1; $i<=3; $i++) {
		...
		if($msg) {
			...
			if($sw_direct == "wish") {
				$str .= "<span class=\"wish_btn\"><a href=\"javascript:ajax_item_wish();\" class=\"btn_large".$sw_css."\"><span>".$sw_name."</span></a></span>";
			}
			else if($sw_direct == "buy") {
				$str .="<span class=\"$sw_direct\"><a href=\"javascript:$sw\" class=\"".$sw_css." btn_large\">".$sw_name."</a></span>";
			}
			else {
				//$str .= "<span><a href=\"javascript:fbuyform_submit('".$sw_direct."');\" class=\"btn_large".$sw_css."\">".$sw_name."</a></span>";
				$str .= "<div class=\"".$sw_position."\"><a href=\"javascript:".$sw."\" class=\"btn_large".$sw_css."\">".$sw_name."</a>";
				$str .= "	<div class=\"fbuyform_pop\" style=\"display:none;\">";
				$str .= "		<div>";
				$str .= "			<span class=\"fbuyform_pop_txt\">장바구니에 상품을 담았습니다.</span>";
...
function get_bank_account($name, $selected='')
{
	...
	$str  = '<label><select id="'.$name.'" name="'.$name.'">'.PHP_EOL;
...
function get_change_select($name, $selected='', $event='')
{
	...
	if(!in_array($selected, array(2,3,4,5,10,11,12,13))) {
	...
	if($subj_count > 1) {
		...
		for($i=0; $i<$subj_count; $i++) {
			...
			if($opt_count) {
				...
				$select  = '<label><select id="it_option_'.$seq.'" class="it_option wfull pro_opt"'.$disabled.'>'.PHP_EOL;
		...
		$select  = '<label><select id="it_option_1" class="it_option wfull pro_opt">'.PHP_EOL;
		$select .= '<option value="">(필수) 선택하세요</option>'.PHP_EOL;
		for($i=0; $row=sql_fetch_array($result); $i++) {
			if($row['io_price'] >= 0)
...
function get_sendcost_amt()
{
	...
	if($gs['sc_type']=='0') {
		...
		if(in_array($delivery_method, array('3','4'))) {
			if($gs['sc_method'] == 1)
				$str = '상품수령시 결제(착불)';
			else if($gs['sc_method'] == 0) { //20200702 선결제만 기능 추가
...
// 분류별 상단배너2
function get_category_head_image2($ca_id)
{
	$cgy = array();
	...
	if(is_file($file) && $row['headimg']) {
		...
		$file = rpc($file, TB_PATH, TB_URL);
		$cgy['headimg'] = '<div id="sub_bn_wrap">'.$a1.'<div class="sub_bn_box" style="background:url('.$file.') no-repeat center; background-size: cover;"></div>'.$a2.'</div>';
	}
...
function get_search_date($fr_date, $to_date, $fr_val, $to_val, $is_last=true)
{
	...
	$frm[] = '<input type="button"'.$js.' class="btn_small white" value="이번달">';
...
// 킬딜특가상품 가져오기
function get_kill_deal_pro($result)
{
	$Date = date("Y-m-d");
	...
	// $row = sql_fetch($sql);
	// $total_count = $row['cnt']; // 2

	// $mod = 3; // 가로 출력 수
	// $rows = $page_rows ? (int)$page_rows : ($mod*10); // 30
	// $total_page = ceil($total_count / $rows); // 전체 페이지 계산
	// if($page == "") {$page = 1;} // 페이지가 없으면 첫 페이지 (1 페이지)
	// $from_record = ($page - 1) * $rows; // 시작 열을 구함

	// $sql = " select * $sql_common $sql_order limit $from_record, $rows";
	// //$sql = select * 킬딜특가상품 중 종료날짜순으로 정렬하여 0행부터
	// $result = sql_query($sql);
