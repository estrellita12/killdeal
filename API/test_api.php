...
$res = sql_fetch("select gs_id from recommend_good where pt_id = '$pt_id'");

$list_best = unserialize(base64_decode($res['gs_id']));
//echo count($list_best);
//var_dump ($list_best);

for($i=0; $i<count($list_best); $i++) {
	if($list_best[$i]['subj'] != $_GET['subj']) continue;
...
for($i=0; $row=sql_fetch_array($result); $i++) {
	...
	echo "<br>";
	*/
	/*$good = array("img" => TB_DATA_URL.'/goods/'.$row['simg1'], "name" => $row['gname'], "normal_price" => $row['normal_price'],
