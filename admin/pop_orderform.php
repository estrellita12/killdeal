...
<div id="sodr_pop" class="new_win">
	...
	<section id="anc_sodr_list">
		...
		<div class="tbl_head01">
			...
			<tr class="<?php echo $bg; ?>">
				<td>
				    <!-- 상품코드_현대리바트_20190715 -->  
				    <input type="hidden" name="item_cd[<?php echo $i; ?>]" value="<?php echo $row['gs_id']; ?>">
					<input type="hidden" name="goods_price[<?php echo $i; ?>]" value="<?php echo $row['goods_price']; ?>">
					<input type="hidden" name="use_point2[<?php echo $i; ?>]" value="<?php echo $row['use_point2']; ?>">
					<input type="hidden" name="use_price[<?php echo $i; ?>]" value="<?php echo $row['use_price']; ?>">
					<input type="hidden" name="sum_qty[<?php echo $i; ?>]" value="<?php echo $row['sum_qty']; ?>">
					<input type="hidden" name="item_nm[<?php echo $i; ?>]" value="<?php echo get_text($gs['gname']);?>">
					<input type="hidden" name="mda_gb[<?php echo $i; ?>]" value="<?php echo $row['od_moible'];?>">
					<input type="hidden" name="deli_amt[<?php echo $i; ?>]" value="<?php echo $row['baesong_price'] + $row['baesong_price2']; ?>">
					<input type="hidden" name="taxsave_yes[<?php echo $i; ?>]" value="<?php echo $row['taxsave_yes']; ?>"> <!-- 현금영수증 사용여부 -->
					<input type="hidden" name="tax_hp[<?php echo $i; ?>]" value="<?php echo $row['tax_hp']; ?>"> <!-- 현금영수증 핸드폰번호 -->
					<input type="hidden" name="tax_saupja_no[<?php echo $i; ?>]" value="<?php echo $row['tax_saupja_no']; ?>"> <!-- 현금영수증 사업자번호 -->
					<input type="hidden" name="magam_uresult[<?php echo $i; ?>]" value="<?php echo $row['magam_uresult']; ?>"> <!-- 포인트사용마감 성공여부 -->
					<input type="hidden" name="cash_uresult[<?php echo $i; ?>]" value="<?php echo $row['cash_uresult']; ?>"> <!-- 현금영수증 신청 성공여부 -->

					<!-- 할인금액 항목이 부재(추후 추가 필요할수도 있음.) -->
                    <!-- 아래에 항목 추가 필요 ex. use_price ,배송비 등 -->
					<!-- 골프유닷넷 포인트 연동_20191220 -->
                    <input type="hidden" name="use_point[<?php echo $i; ?>]" value="<?php echo $row['use_point']; ?>">
					<input type="hidden" name="sum_point[<?php echo $i; ?>]" value="<?php echo $row['sum_point']; ?>">
                ...
                <!-- (2021-04-05) -->
                 <td title="<?php echo get_order_change_log('',$row['od_no']); ?>">
					<?php echo get_change_select("change_status[".$i."]", $row['dan']); ?>
					<?php if(in_array($row['dan'], array(7,9)) && $row['refund_price'] == 0 && in_array($row['paymethod'], array('신용카드', 'KAKAOPAY'))) { ?>
					...
					<?php }else if(in_array($row['dan'], array(7,9)) && $row['refund_price'] == 0 && in_array($row['paymethod'], array('계좌이체', '가상계좌'))){ ?>
					<p class="padt3"><span id="order_cancel_account" data-id=<?php echo $row['od_no']?> data-price=<?php echo $row['use_price'] ?> class="btn_ssmall red">PG부분취소</span></p>
					<?php } ?>
				</td>
				...
				<td class="tar"><?php echo number_format($row['use_point2']); ?></td>
	...
	if($od['paymethod'] == '간편결제') {
		if($od['od_pg'] == 'lg')
			...
			$s_receipt_way = $od['paymethod'];
        $s_receipt_way = "PG간편결제";
	}
	if($amount['usepoint'] > 0)
		$s_receipt_way .= "+포인트";

    // (2021-03-04)
	if( $od['pt_id']=='golf' && $amount['usepoint2'] > 0 ) {
		if( $amount['useprice']==0 ){
            $s_receipt_way = "기본금";
        }else{
            $s_receipt_way .= "+기본금";
        }
		...
		<div class="compare_wrap">
			<section id="anc_sodr_chk" class="compare_left">
				...
				<div class="tbl_frm01">
					...
					<!-- 현대리바트 기본금 -->
					<tr>
						<th scope="row">기본금결제(현대리바트)</th>
						<td class="td_price">(-) <?php echo display_price($amount['usepoint2']); ?></td>
					</tr>
					<!-- end -->
					<tr>
					...
					<tr>
						<th scope="row" class="fc_red">환불취소액(PG승인취소)</th>
						<td id="refund_result" class="td_price bg1 fc_red">(-) <?php echo display_price( $amount['cancel']+$amount['refund']); ?></td>
					</tr>
					<tr>
