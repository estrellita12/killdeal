...
<ul class="anchor">
	...
	<li><a href="#price_change_list">가격변경내역</a></li>
</ul>
EOF;
?>
...
<div class="tbl_frm02">
	...
	<tr>
		...
		<td>
            ...
            <?php echo help('상품 관련 이미지 저장시 이미지가 저장될 폴더 이름'); ?>
		</td>
	</tr>
	<tr>
		<th scope="row">상품코드2(gs_id)</th>
		<td>
			<input type="text" name="index_no" value="<?php echo $gs['index_no']; ?>" required itemname="상품코드" class="required frm_input"<?php echo $readonly; ?>>
			<?php if($w == "u") { ?><a href="<?php echo TB_SHOP_URL; ?>/view.php?index_no=<?php echo $gs_id; ?>" target="_blank" class="btn_small">미리보기</a><?php } ?>
            <?php echo help('상품 ID (상품 구별 코드)'); ?>
		</td>
	</tr>
	<tr>
		<th scope="row">상품명</th>
	...
	<tr>
		<th scope="row">사이드배너 진열</th>
		<td class="td_label">
			<?php echo radio_checked('sidebanner', $gs['sidebanner'], '1', '노출'); ?>
			<?php echo radio_checked('sidebanner', $gs['sidebanner'], '0', '비노출'); ?>
		</td>
	</tr>
	<tr>
		<th scope="row">장바구니추천 진열</th>
		<td class="td_label">
			<?php echo radio_checked('recomm_use', $gs['recomm_use'], '1', '노출'); ?>
			<?php echo radio_checked('recomm_use', $gs['recomm_use'], '0', '비노출'); ?>
		</td>
	</tr>
	<tr>
...
<div class="tbl_frm02">
	...
	<tr>
		...
		<td>
			...
			<input type="text" name="sb_date" value="<?php echo $gs['sb_date']; ?>" id="sb_date" class="frm_input w80" maxlength="10" autocomplete="off"> ~
			<label for="eb_date" class="sound_only">종료일</label>
			<input type="text" name="eb_date" value="<?php echo $gs['eb_date']; ?>" id="eb_date" class="frm_input w80" maxlength="10" autocomplete="off">
			<a href="javascript:void(0);" class="btn_small is_reset">기간초기화</a>
			<br><br>
			<span style="display:block; padding-bottom:5px;">
				<label for="main_use_yes">메인 노출</label>
				<input type="checkbox" name="main_use" value="1" id="main_use_yes" <?php echo get_checked($gs['main_use'], "1"); ?>>
			</span>
			<span>
				<label for="">순서</label>
				<input type="text" name="main_order" value="<?php echo $gs['main_order']; ?> " class="frm_input"> 숫자가 작을수록 우선 순위로 노출 됩니다.
			</span>
			<br>
...
function checkAll(obj){
    var res = $(obj).prop("checked");
    if(res)  $("input[name='use_hide[]']").prop("checked",true);
    else  $("input[name='use_hide[]']").prop("checked",false);
}
</script>
            <label class="w150 dib fs12"><input type="checkbox" onclick="checkAll(this)"> 전체</label>
            <?php
            $ptList = get_partner_list();
            foreach( $ptList as $pt ){
                $ptName = get_member($pt, 'name');
                $checkList = explode(",",$gs['use_hide']);
                $checkList = array_filter($checkList);
                $checked = false;
                if( in_array($pt,$checkList) ) $checked = true;
                $str = '<label class="w150 dib fs12"><input type="checkbox" name="use_hide[]" value="'.$pt.'"';
                if($checked) $str.= ' checked="checked"';
                $str.= '> '.$ptName['name'].'</label>'.PHP_EOL;
                echo $str;
            }
            ?>
        </td>
...
// 테이블의 전체 레코드수만 얻음
$total_count = sel_count("shop_goods_price_change_log","where gs_id=$gs_id");
$rows = 5;
...
//$sql ="select * from shop_goods_price_change_log  where del_yn='N'  and gs_id = '$gs_id' order by index_no desc limit $from_record, $rows ";
$sql ="select * from shop_goods_price_change_log  where del_yn='N'  and gs_id = '$gs_id' order by index_no desc limit 5";
$result = sql_query($sql);
?>	
<? for($i=0; $row=sql_fetch_array($result); $i++){?>
	<tr>
		...
		<td><?php echo $row['change_text']?></td>
...
<?php
//echo get_paging($config['write_pages'], $page, $total_page, $_SERVER['SCRIPT_NAME'].'?'.$q2.'&bak='.$bak);
?>
</section>
<?php } ?>
<?php echo $frm_submit; ?>
