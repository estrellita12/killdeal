...
<div class="tbl_head01">
	...
	<tr>
		...
		<th scope="col">번호</th>
		<th scope="col"><?php echo subject_sort_link('iq_ty',$q2); ?>문의유형</a></th>
	...
	<tr class="<?php echo $bg; ?>">
        ...
        <td><a href="<?php echo TB_SHOP_URL; ?>/view.php?index_no=<?php echo $row['gs_id']    ; ?>" target="_blank" class="fs11"><?php echo $gs['gname']; ?></a></td>
        <td class="tal">
            <p><?php echo cut_str($row['iq_subject'],70); ?></p>
            <div class="dn" style="background-color:#fdfbf5">
                <table>
                    ...
                    <tr>
                        <td class="fs11">질문</td>
                        <td class="fs11" style="text-align:left"> <?php echo nl2br($row['iq_question']); ?> </td>
                    </tr>
                    <tr>
                        <td class="fs11">답변</td>
                        <td class="fs11" style="text-align:left"><?php echo nl2br($row['iq_answer']); ?></td>
                </table>
            </div>
        </td>

		<td><?php echo trans_pt_name($row['pt_id']); ?></td>
		<td><?php echo $iq_name ?><br><span style="color:#888">(<?php echo $row['mb_id'] ?>)</span></td>
...
$(function(){
    $("#frmHelpExcel").on("click", function() {
        ...
        $el_chk.each(function(index) {
            if($(this).is(":checked")) {
                help_chk.push($("input[name='iq_id["+index+"]']").val());
            }
        });
        ...
        if(help_no == "") {
            ...
            this.href = "./goods/goods_qa_excel.php?help_no="+help_no;
            return true;
        }
    });
