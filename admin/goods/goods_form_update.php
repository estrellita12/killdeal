...
$value['brand_nm']		= $_POST['brand_nm']; //브랜드명 //20220210
...
$value['main_use']	= $_POST['main_use']; //메인노출
$value['main_order']	= $_POST['main_order']; //메인노출순서
$value['sidebanner']	= $_POST['sidebanner']; //사이드배너 진열
$value['recomm_use']	= $_POST['recomm_use']; //사이드배너 진열
// 2023-04-11 -----------------------------
if( !empty($_POST['use_hide']) ){
    $useHideList = array_filter($_POST['use_hide']);
    if( !empty($useHideList) ) $value['use_hide'] = implode(",",$useHideList);
    else $value['use_hide'] = "";
}else{
    $value['use_hide'] = "";
}
...
if($w == "") {
	...
	//로그 남기기 위한 변수 --20200903  (2021-02-24)
	...
	goods_price_update($gs_id, $mb_id, $normal_price, $supply_price, $goods_price, $gname,$memo,$isopen);
}
