...
for($i=2; $row=sql_fetch_array($result); $i++)
{
	...
	->setCellValueExplicit($char++.$i, $row['readcount'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
    ...
    $excel -> getActiveSheet()->getStyle("R$i:T$i") -> getNumberFormat() -> setFormatCode("#,##0");
}

// Rename worksheet
$excel->getActiveSheet()->setTitle('상품목록');
...
$excel->getActiveSheet()->getStyle("A1:AT1")->applyFromArray($TD_COLOR);
...
header('Content-Disposition: attachment; filename="상품목록-'.date("ymd", time()).'.xlsx"');
