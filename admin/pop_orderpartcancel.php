...
$cancel_amount = get_order_spay($od_id,'and dan in (7,9)'); // 결제정보 합계
$od_show_price = $cancel_amount['useprice'] - $cancel_amount['refund'];

$cancel_amount = get_order_spay($od_id,"and od_no='$od_no'"); // 결제정보 합계
$od_holder_price = $cancel_amount['useprice'] - $cancel_amount['refund'];
...
<div class="new_win">
    ...
    <div class="od_test_caution">주의) 취소 요청 금액 입력시 실결제액, 배송비, 포인트결제를 반드시 확인해 주세요. </div>
    <div class="tbl_frm01 tbl_wrap">
        ...
        -->
        <tr>
            <th scope="row">주문 총액</th>
            <td><?php echo display_price($od_receipt_price); ?> </td>
        </tr>
        <tr>
            <th scope="row">취소 가능 금액</th>
            <td><?php echo display_price($od_show_price); ?> </td>
        </tr>

        </tr>
        <tr>
            <th scope="row"><label for="mod_tax_mny">취소 요청 금액</label></th>
            <td><input type="text" name="mod_tax_mny" value="" id="mod_tax_mny" class="frm_input" placeholder="<?php echo number_format($od_holder_price); ?>" > 원</td>
        </tr>
        <!--
        <tr>
